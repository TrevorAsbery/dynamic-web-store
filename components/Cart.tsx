...
const Cart = () => {
  ...
  return (
    <div>
      {/* Cart Button */}
      <button onClick={() => setIsOpen(!isOpen)} className="fixed top-4 right-4 bg-blue-500 text-white p-2 rounded z-50">
        🛒 ({items.length})
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 isolate z-50">
          ...
          {/* Cart Panel */}
          <div className="fixed top-0 right-0 w-80 h-full bg-white shadow-lg p-4 border-l">
